    ...
    try {
        ...
        while (pageCursor !== null && pageCursor !== undefined) {
            ...
            const response = await fetch(
                apiURL + query + filter + "&per-page=200&cursor=" + pageCursor,
                {
                    method: "GET",
                    headers: {},
                }
        ...
        for (const openAlexPaper of outputJSON) {
            ...
            const url =
                openAlexPaper.open_access.oa_status === "closed"
                    ? openAlexPaper.id
                    : openAlexPaper.open_access.oa_url;
        ...
        LogMessage(
            "Error while searching OpenAlex paper " +
                paperID +
                ":\n" +
 ...
 */
export async function getOpenAlexPaperID(
    title: string
    ...
    try {
        ...
        const response = await fetch(
            apiURL + searchQuery + title + "?per-page=1",
            {
                method: "GET",
                headers: {},
            }
        ...
        LogMessage(
            "Error while fetching paperID of " +
                title +
                " on OpenAlex:\n" +
                (error.message as string),
            ErrorLevel.err
