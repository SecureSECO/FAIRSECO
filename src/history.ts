...
export function getHistoryData(
    ...
    // Get quality score
    const quality = (
        result.find((ele) => ele.ModuleName === "QualityMetrics")?.Data as
            | QualityMetrics
            | undefined
    )?.score;

    // Get citations
    const citationsData = result.find(
        (ele) => ele.ModuleName === "CitingPapers"
    )?.Data as Citations | undefined;
    const firstHandCitations = citationsData?.firstHandCitations;
    const secondHandCitations = citationsData?.secondHandCitations;
    const citations =
        firstHandCitations === undefined || secondHandCitations === undefined
            ? undefined
            : firstHandCitations + secondHandCitations;

    // Get fairness
    const fairnessOutput = result.find((ele) => ele.ModuleName === "fairtally")
        ?.Data as FairtallyOutput[] | undefined;
    const fairness =
        fairnessOutput !== undefined ? fairnessOutput[0].count : undefined;

    // Get method matches
    const searchsecoOutput = result.find(
        (ele) => ele.ModuleName === "SearchSECO"
    )?.Data as Output | undefined;
    let matches;
    ...
    if (
        ...
        historyData[historyData.length - 1].date === date
    ) {
        ...
        if (
            newData.quality !== undefined ||
            newData.citations !== undefined ||
            newData.fairness !== undefined ||
            newData.fairness !== undefined
        ) {
    ...
    try {
        ...
        LogMessage(
            "Failed to write to FAIRSECO history file:\n" +
    ...
    try {
        ...
        for (const data of readData) {
            if (typeof data !== "object") {
                throw new Error(
                    "Incorrect format (element of history is not an object)"
                );
            }
        }
        ...
        if (error.code === "ENOENT") {
            // File not found
            LogMessage(
                "No FAIRSECO history file found:\n" + (error.message as string),
                ErrorLevel.info
            );
        } else {
            // Error reading history file
            LogMessage(
                "Failed to read FAIRSECO history file (history will be reset!):\n" +
                    (error.message as string),
                ErrorLevel.warn
            );
        }
